...
| 通知删除（delete，Bark 兼容） | 推送请求带真值 `delete` + `id`（正整数）时**短路整条推送链路**（忽略其它推送参数、不写监控流）：鸿蒙目标调用华为 v1 `messages:revoke` 撤回相同 notifyId 的通知，iOS 目标经 Bark 静默推送删除相同 id 的通知（通知中心 + App 历史记录）；任一平台成功即 200；`id` 必须为正整数否则 400。鸿蒙撤回端点用应用级 `clientID`（与发送的 v3 `projectId` 不同，在 `harmony/harmony_certs.go` 配置，留空不影响发送）；V1/V2/批量/MCP 全入口支持；平台扇出后服务端同步删除该 key 下 `extras.id` 相同的历史消息，并向删除流水写一条按 `extras.id` 删除的记录（`deletions` 信封新增 `extraIds`），供已同步的客户端下轮同步时删除本地副本 | [API.md](docs/API.md) |
