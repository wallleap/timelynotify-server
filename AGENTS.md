...
- 华为**消息删除/撤回**（push revokes）：推送请求带真值 `delete` 参数时走删除分支，与发送链路差异如下——**端点是 v1** `https://push-api.cloud.huawei.com/v1/{clientId}/messages:revoke`（**不是**发送的 v3 projectId），URL 里的**应用级** Client ID 存在 `harmony/harmony_certs.go` 的 `clientID` 变量（AGC「项目设置→常规→应用信息→OAuth 2.0客户端ID(凭据)-Client ID」，值=APP ID；**不是** `agconnect-services.json` 顶层 `client.client_id`——那是项目级 Client ID，填错报 80300002 "No permission to send message to these tmIDs"；留空不影响发送，撤回快速报错）；**请求体是扁平结构** `revokeMessage{notifyId int, token []string}`（`harmony/client.go`），**不是** v3 的嵌套 payload/target；头同样是 Bearer JWT + `push-type:0`，成功码同为 80000000、80200003 过期重试一次（`withRetry`/`postOnce` 为 send/revoke 共用）。入口侧：`push()` 把 `delete` 标志（V1 query/form 字符串、V2 JSON bool/数字/字符串）经 `isTruthyFlag`（纯函数，可测）归一化为 `ExtParams["delete"]="1"`（原始键删除，`delete:0`/`false` 等 falsey 值不透传 APNs 载荷），在 device_key 校验后、`gotifyPublish` 前**短路**进 `pushDelete()`——**双平台删除**：鸿蒙目标调华为 v1 `messages:revoke` 撤回相同 notifyId，iOS 目标走原生静默推送（`apns.Push` 在 `IsDelete` 时构建 ContentAvailable + `push-type: background`，Bark App 删除同 id 通知及历史记录）；`id` 必须解析为正整数（否则 400 且不调任何通道——Bark App 与华为 revoke 都靠 `id` 定位）、**忽略其它所有推送参数**、**不写 gotify 监控流**；任一通道成功即 200，全部失败才 500，无任何有效目标（含 platform 收窄后）返回 400；80200001/80300007 按 `ClearDeviceTokenByKeyAndPlatform(key,"harmony")` 清死 token。撤回 seam 变量 `revokeHarmony`（与 `pushHarmony`/`pushAPNs` 同级）供测试替换；共享核心 `revokeHarmonyTokens`（id 校验/客户端检查/撤回调用/死 token 清理）只服务鸿蒙目标；MCP 入口因共用 `push()` 自动支持。**历史同步删除**：平台扇出之后（无论成败）`pushDelete` 还会 best-effort 调 `gotifyService.DeleteMessageByExtraID(device_key, id)`（失败仅 Warn 不影响响应码）——删除该 key 下 `extras.id` 相同的历史消息（命中时同事务写 kind=1 墓碑），并无条件追加一条 kind=4（`DeletionByExtraID`）流水；`deletions` 信封新增 `extraIds` 数组（int64），已同步客户端下轮 `deletedSince` 据此删本地副本（历史按 device_key 共享，`platform` 收窄不影响清理）。**端侧生效差异（实测）**：撤回返回 80000000 只表示 Push 服务端受理，已展示通知的移除需真机 HarmonyOS NEXT（Phone/Tablet/PC 5.1+）；**DevEco 模拟器不处理已展示消息的撤回指令**（API 返回成功但通知保留，未下发消息仍可拦下）——模拟器上"撤回成功通知还在"是平台限制不是 bug，端到端验证撤回效果必须用真机。
